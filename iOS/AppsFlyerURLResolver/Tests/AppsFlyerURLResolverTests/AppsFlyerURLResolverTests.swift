...
final class AppsFlyerURLResolverTests: XCTestCase {
    ...
    func testResolveMILink() throws {
        ...
        waitForExpectations(timeout: 2) { error in
          XCTAssertNil(error)
          XCTAssertEqual("https://www.gap.com/?mi_u=%25%25email_key%25%25&EV=%25%25EV_value%25%25&DI=%25%25DI_value%25%25&CD=%25%25CD_value%25%25&cvosrc=email.exacttarget.GPUS04162021", res)
        }
      }
    ...
    func testExtractLastLinkFromJSCode() {
        ...
        guard let filePath = Bundle(for: self.classForCoder).path(forResource: "HubspotHTMLValidURL", ofType: "html"), let fileContent = Helpers.htmlFileToString(for: filePath) else {
            XCTFail("No testing file in bundle found")
            return
        }
        
        
        let lastLink = jsResolver.extractLinkFrom(JSCode: fileContent)
        print(">>Got link! \(lastLink ?? "")")
        XCTAssertEqual(lastLink, expectedLink)
    }
    
    func testURLResolver_extractLingFrom_EmersysLinkFound() {
        let expectedLink = "https://yossiesp.onelink.me/Amo7?pid=yossiemarsys&amp&c=ESPTEST&amp&af_force_deeplink=true&amp&is_retargeting=true"
        guard let filePath = Bundle(for: self.classForCoder).path(forResource: "EmersysHTMLExampleSuccess", ofType: "html"),
              let fileContent = Helpers.htmlFileToString(for: filePath) else {
            XCTFail("No testing file in bundle found")
            return
        }
    ...
    func testAppsFlyerURResolver_resolveEmersysLink_happyFlow() {
        var res : String?
        let expectation = self.expectation(description: "Waiting for the resolve call to complete.")
        URLResolver().resolveJSRedirection(url: "https://esp2.esp-integrations.com/u/nrd.php?p=6p7NTHUaLG_2_100288193_1_1&ems_l=100119277&_esuh=_11_47d316c3d7023551582485e7a4a92d4d67b13783ce907f91f7234a99aa540d3e", completionHandler: { url in
            res = url
            expectation.fulfill()
          })
          waitForExpectations(timeout: 2) { error in
            XCTAssertNil(error)
            XCTAssertEqual("https://yossiesp.onelink.me/Amo7?pid=yossiemarsys&amp;c=ESPTEST&amp;af_force_deeplink=true&amp;is_retargeting=true", res)
          }
    }
